...
score_map_real = np.ones (shape=(1,16,16,1))
score_map_fake = np.zeros(shape=(1,16,16,1))
...
fake_b_score, real_b_score, \
               fake_a_score, real_a_score, \
               fake_b_cyc_a, fake_a_cyc_b = CycleGAN()(inputA,inputB)
cyclegan = tf.keras.models.Model(inputs=[inputA, inputB], outputs=[fake_b_score, real_b_score, fake_a_score, real_a_score, fake_b_cyc_a, fake_a_cyc_b])
# cyclegan.summary()

cyclegan.compile( # optimizer ="adam", 用了這個好像也沒用
                 loss = ["mse","mse","mse","mse","mse","mse"] ### 隨便亂設定的
                 )
tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir="logs")
cyclegan.fit(x = [imgA, imgB],
             y = [score_map_fake, score_map_real, score_map_fake, score_map_real, imgA, imgB],
